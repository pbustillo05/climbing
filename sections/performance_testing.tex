...
\subsubsection{Peak Force Asymmetry}

Left--right isometric finger force asymmetry is measured by comparing sequential single-arm peak pulls on the same hold and body position---the Tindeq Progressor is the most accessible tool for this purpose.
Lattice Training\singlesource\ analysed their client dataset and reported a mean left--right finger strength difference of approximately 1.6~kg across their climber population, indicating that most climbers are nearly symmetric in absolute terms.
No peer-reviewed study has used prospective cohort design to link a specific asymmetry magnitude to pulley injury incidence.

\begin{bioplausbox}{Force Asymmetry as Injury Risk Indicator · \bioplausible · ESS 2}
Mechanistic rationale: large bilateral asymmetries alter load distribution between hands during symmetric or near-symmetric moves, potentially concentrating cumulative stress on the weaker-side finger pulleys.
The $>$10--15\% inter-limb threshold cited in lower-extremity injury prediction literature has been applied to fingers by some practitioners, but no climbing-specific prospective study has quantified injury incidence relative to measured asymmetry magnitude.
Longitudinal tracking of bilateral asymmetry is low-risk and may provide early signal of unilateral overloading; actionable thresholds remain undefined pending prospective data.
\end{bioplausbox}
...
\medskip
\noindent\textbf{Grip technique and edge depth.} Half-crimp on a 20~mm edge (\toverify; de facto research and applied standard via Lattice Training and academic protocols) yields $R^{2} = 0.48$--$0.58$ for bouldering in a 6-technique comparison \cite{BMK+25}, the highest of any single technique tested. Grant et al.\ \cite{GHWA96} corroborate finger strength as a key discriminator between ability groups. Importantly, MacKenzie et al.\ \cite{TEMP_MacKenzie2020} present a notable counterpoint: in 44~male and 33~female climbers spanning 5a--8a onsight, shoulder power and endurance (max pull-ups, arm crank power, bent-arm hang) emerged as the \emph{primary} independent predictors (adj.\ $R^{2} = 0.77$ males, $0.62$ females) while finger strength trended toward but did not enter the final model---suggesting that at moderate grades, sustained pulling capacity may be more discriminant than peak finger strength; this finding should be weighted against the possibility that the pull-up tests partly proxy finger loading and that the grade range studied (5a--8a) does not reach the levels where maximal finger strength typically becomes the bottleneck (\toverify; DOI unconfirmed).
...
Mermier et al.\ \cite{MJPS00} & 44 & 0.66 & Training component 58.9\%; flexibility component 1.8\%; anthropometric component (ape index, height, mass) 0.3\%; remaining $\sim$5.4\% minor components. Finger strength not separately isolated; training volume proxies accumulated finger loading \\[3pt]
...
Pull-up strength is a moderate predictor of bouldering performance: Buraas and Stien \cite{BBW25} report $r = 0.55$ for bouldering grade ($n = 19$), secondary to finger strength ($r = 0.89$). For sport climbing at the elite level, one \toverify source\singlesource\ (Lattice Training 2025 performance data blog post) reports that the 2-repetition-maximum pull-up was the strongest single predictor in a route-climbing model ($R = 0.521$, adj.\ $R^{2} = 0.253$), potentially reflecting the greater importance of sustained upper-body endurance on long routes. This finding is blog-level only and must not be treated as settled guidance; a peer-reviewed analysis of the same dataset has not been published.

MacKenzie et al.\ \cite{TEMP_MacKenzie2020} provide the most striking pull-up evidence: shoulder power and endurance (maximum pull-ups, arm crank power, bent-arm hang) explained adj.\ $R^{2} = 0.77$ of climbing ability in males and $0.62$ in females across a 5a--8a onsight sample, with finger strength entering only as a trend variable. The investigators reported that a test-of-principle training intervention increasing these determinants by 42--67\% produced a 2--3 grade improvement, constituting the strongest available causal evidence for pulling capacity as a grade driver, though full study details have not been confirmed from the available abstract (\toverify; DOI unconfirmed). Pull-up training and testing protocols are addressed in detail in the physical test battery section.
...
\subsubsection{Slope of Force Decline and Fatigue Indices}

The slope of force decline (SoD) quantifies the rate at which peak force drops across successive repetitions during an intermittent endurance test.
It is computed as the linear regression slope of peak force per repetition against repetition number (units: N/rep or \%MFS/rep) from protocols such as the stamina test used by Stien et al.\ ($24 \times 10$\,s on / 6\,s off at 80\,\% MFS on a 12~mm hold; \cite{DO22}).
Force--time data exported from the Tindeq Progressor are sufficient for SoD calculation without specialised laboratory equipment.

\begin{bioplausbox}{Slope of Force Decline as Endurance Index · \bioplausible · ESS 3}
SoD is mechanistically linked to W$'$ depletion rate and local buffering capacity but has not been validated as an independent performance predictor after controlling for CF and MFS.
The Jullien et al.\ systematic review \cite{TEMP_Jullien2025} noted that fatigue index (rate of force decline) failed to discriminate climbing ability in at least one large reliability study, whereas MFS and CF showed significant associations.
No standardised MDC has been published for SoD; it should be treated as a research metric pending further validation rather than a routine monitoring variable.
\end{bioplausbox}

\subsubsection{Near-Infrared Spectroscopy (NIRS) and Muscle Oxygenation}

Near-infrared spectroscopy (NIRS) measures forearm flexor muscle oxygenation (SmO$_2$) non-invasively, providing a real-time window into local aerobic capacity, forearm blood flow, and fatigue kinetics during climbing-specific tasks.
Wearable sensors (Moxy Monitor, BSX Insight) are positioned over the flexor digitorum profundus; published reliability for intermittent contraction tasks in sport climbers is generally ICC $>$ 0.75 when device placement is standardised.

\medskip
\noindent\textbf{Performance prediction.}
Fryer et al.\ \cite{FSS+16} measured forearm oxidative capacity index (NIRS-derived) during a standardised intermittent hang protocol and found it significantly predicted self-reported sport climbing ability.
A combined model of aerobic predictors---forearm oxidative capacity, treadwall maximal deoxygenation, and treadwall VO$_2$peak---accounted for approximately 67\,\% of variance in route redpoint ability.
This explanatory power is comparable to MFS-only models in the same sample.

\begin{evidencebox}{Forearm Oxidative Capacity as Route-Climbing Predictor · \evidencebacked · ESS 5}
Fryer et al.\ (2016) \cite{FSS+16}: forearm oxidative capacity index (NIRS-derived) predicted sport climbing grade; combined aerobic NIRS model explained ${\approx}67\,\%$ of route-performance variance.
Design: cross-sectional, $n$ not specified in abstract; outcome: self-reported redpoint grade.
Limitations: single study, small probable sample, outcome is self-report, no simultaneous MFS covariate, sport climbing only.
This finding establishes NIRS-measured aerobic forearm capacity as a grade predictor, but the incremental predictive value beyond MFS alone has not been isolated in a multivariate model.
\end{evidencebox}

\medskip
\noindent\textbf{Acute vs.\ cumulative fatigue monitoring.}
SmO$_2$ drops reliably within a single maximal hang, reaching a near-minimum plateau at task failure regardless of intensity.
However, Dindorf et al.\ (\toverify; peer-reviewed NIRS fatigue monitoring study, DOI unconfirmed) reported that SmO$_2$ did not change significantly \emph{across} three repeated maximal hangs, indicating that SmO$_2$ level does not track cumulative session fatigue between separate bouts.
Termination SmO$_2$ (the saturation level at task failure within a single contraction) showed less variability in climbers with longer hang times, suggesting a diagnostic role for acute bout monitoring.
Baláš et al.\ \cite{BGJ+24} additionally noted that during CF-level effort, muscle oxygenation parameters could not clearly distinguish exhaustive from non-exhaustive trials, limiting utility for CF threshold determination via NIRS alone.

\begin{bioplausbox}{NIRS-Guided Inter-Set Rest Prescription · \bioplausible · ESS 2}
Mechanistic rationale: SmO$_2$ recovery kinetics during rest between hangboard sets reflect forearm reperfusion rate, which determines readiness for the next effort above CF.
Using SmO$_2$ recovery as a rest-length criterion (e.g., resume when SmO$_2$ returns to $\geq$80\,\% baseline) is physiologically coherent and has been proposed in coaching contexts, but no climbing-specific RCT has tested NIRS-guided rest titration against fixed-rest protocols for performance or adaptation outcomes.
\end{bioplausbox}
...
Mermier et al.\ \cite{MJPS00} & 44 mixed-sex (5.6--5.13c) & PCA + stepwise regression & 0.66 & Training component 58.9\%; flexibility 1.8\%; anthropometrics 0.3\%; remaining $\sim$5.4\% minor components. Finger strength not isolated --- training volume proxies cumulative finger loading \\[3pt]
...
Magiera et al.\ \cite{TEMP_Magiera2013} & 30 advanced males (7b$+$ OS, 8a RP mean) & Canonical discriminant analysis & 0.77 & Canonical $R = 0.935$ ($p < 0.001$); 7 predictors entered as canonical weights: relative finger strength (0.490), mental endurance ($-$0.410), climbing technique (0.370), isometric finger endurance (0.340), reaction-time errors ($-$0.319), ape index ($-$0.319), arm VO$_2$ at anaerobic threshold (0.254). DOI candidate 10.2478/hukin-2013-0010 (\toverify) \\[3pt]

MacKenzie et al.\ \cite{TEMP_MacKenzie2020} & 44M + 33F (5a--8a) & Multiple regression & 0.77 (M), 0.62 (F) & Shoulder power and endurance (max pull-ups, arm crank power, bent-arm hang) emerged as the \emph{primary} determinants in the final model; finger strength showed trends toward significance but did not enter as a primary independent predictor. Interpretation: at this grade range, shoulder endurance may be limiting before finger strength becomes discriminant; DOI unconfirmed (\toverify) \\[3pt]
...
\paragraph{Boulder2Vec: Probabilistic Matrix Factorisation on IFSC Competition Data.}
Baron et al.\ \cite{Baron2024Boulder2Vec} applied Probabilistic Matrix Factorisation (PMF) --- a recommender-system framework --- to IFSC professional bouldering competition data (2008--2022). Climbers and boulder problems are represented as latent multi-dimensional vectors; performance is predicted via dot product, capturing both overall climber strength and problem-type specialisation. PMF outperformed logistic regression by modelling climbers' heterogeneous skill profiles across problem styles (e.g., static/technical vs.\ powerful/dynamic). The model handles sparse competition datasets where each climber attempts only a subset of problems. This preprint has not undergone peer review but constitutes the most methodologically rigorous application of latent-factor ML to competition-level climbing performance.
...
\begin{enumerate}[leftmargin=1.5em]
  ...
  \item What is the minimal detectable change (MDC) for CF and W$'$ in field settings? Giles et al.\ provide ICCs but not MDC expressed in clinically interpretable units (\%BW or N); coaches cannot determine whether a retest result represents a real change or measurement noise.
  \item Does bilateral finger force asymmetry $>$10\,\% prospectively predict pulley injury incidence? The biological plausibility is established but no longitudinal study has linked asymmetry magnitude to injury incidence in climbers.
  \item Does the optimal edge depth for reliability (20--23~mm) coincide with optimal ecological validity, or do smaller edges ($<$18~mm) better isolate the force ranges encountered on hard boulder problems?
  \item Can NIRS-derived SmO$_2$ recovery kinetics be used to auto-regulate inter-set rest duration during fingerboard training, and does this improve adaptation outcomes relative to fixed rest?
\end{enumerate}
% -------------------------------------------------------------------
