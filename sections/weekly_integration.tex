...
Stien et al.\ (2023) (\textit{Biol Sport} 40(1):179--191; DOI 10.5114/biolsport.2023.113295) was previously flagged as \toverify due to conflicting volume/page descriptors across secondary sources (40(3):875--889 vs.\ 40(1):179--191). CrossRef metadata now confirms 40(1):179--191 as canonical; the bibliography entry has been corrected. Effect-size data (SDM with 95\% CI) are drawn from sources that extracted numeric values with explicit table citation.
...
\begin{longtable}{>{\raggedright}p{3.0cm} >{\raggedright}p{2.4cm} >{\raggedright}p{3.4cm} >{\raggedright}p{2.8cm} >{\raggedright}p{2.6cm} >{\raggedright}p{1.8cm} >{\raggedright\arraybackslash}p{1.8cm}}
\caption{Peer-reviewed studies informing weekly training integration}
\label{tab:weekly_studies}\\
\toprule
\textbf{Study} & \textbf{Design / n} & \textbf{Protocol} & \textbf{Primary outcome} & \textbf{Effect size} & \textbf{Endpoint} & \textbf{Quality} \\
\midrule
\endfirsthead
...
& Finger strength, RFD, forearm endurance (climbing-specific dynamometric tests)
& Overall SDM = 0.57 (95\% CI 0.24–0.91); finger strength SDM = 0.41 (0.03–0.80); RFD SDM = 0.90 (0.21–1.61); forearm endurance SDM = 1.23 (0.69–1.77)
& Surrogate (dynamometric); no on-wall test improvement found
& GRADE: low-to-moderate. PEDro median $\approx$5.8 (range 5–7). \\

López-Rivera \& González-Badillo (2019) \textit{J Hum Kinet} 66:183–194. \cite{LRGB19}
& RCT, 3 active groups; $n = 26$ advanced climbers ($\sim$7c+/8a)
& 8 wk, 2 sessions/wk: MaxHangs (18 mm, $\geq$80\% MFS) vs.\ IntHangs (repeaters) vs.\ Combination. All continued regular climbing 6 d/wk.
& Grip endurance: time-to-failure dead hang on 11 mm edge (bodyweight)
& IntHangs: $+$45\% (ES = 1.0, $p = 0.002$); MaxHangs: $+$34\% (ES = 0.6, $p = 0.010$); Combination: $+$7\% (ES = 0.1, ns)
& Surrogate (dynamometric time-to-failure); no on-wall outcome
& GRADE: low. No allocation concealment; no blinding; $n \approx$8--9/group. \\
...
& 4 wk, 2 sessions/wk: F100 (100\% MFS), F80 (80\% MFS), F60 (60\% MFS), control on 12 mm hold
& MFS (N), stamina (force decline under repeated effort), endurance (time to failure at submaximal load)
& F100 MFS $+$14.3\% ($p < 0.001$); F80 MFS $+$12.4\% ($p < 0.001$); F80 stamina 29.5$\to$60.4\% ($p < 0.01$); F80 endurance 63.2$\to$79.2\% ($p < 0.001$)
& Surrogate (dynamometric force and endurance)
& GRADE: low. 4-wk duration; no blinding; mixed-sex. \\

Stien et al.\ (2021) \textit{J Sports Sci Med} 20:438–447. \cite{SPV+21}
& RCT; $n = 16$ advanced-to-elite climbers (3 groups of $n = 5$, 6, 5)
& 5 wk, campus board: 4$\times$/wk vs.\ 2$\times$/wk (volume equated) vs.\ climbing-only control; $\geq$48 h between sessions
& Bouldering performance, max finger force, campus moves to failure, RFD
& 4$\times$/wk: max force $\uparrow$, RFD $\uparrow$ vs.\ control; 2$\times$/wk: bouldering performance $\uparrow$, campus moves $\uparrow$ vs.\ control; no sig.\ difference between frequency groups
& Mixed: bouldering performance (functional); max force, RFD (surrogate)
& GRADE: very low. $n = 5$--6/cell; 5-wk duration. \\

Wilson et al.\ (2012) \textit{J Strength Cond Res} 26(8):2293–2307. \cite{WMR+12}
& Meta-analysis; 21 RCTs/CCTs; $n = 422$ effect sizes
& Concurrent strength + endurance vs.\ strength-only; running/cycling modalities; varied frequencies and durations
& Hypertrophy, maximal strength, power
& Strength ES: strength-only 1.76 vs.\ concurrent 1.44 ($-$18\%); hypertrophy ES: 1.23 vs.\ 0.85 ($-$31\%); power ES: 0.91 vs.\ 0.55 ($-$40\%); endurance duration $r = -0.29$ to $-0.75$
& Surrogate (isokinetic strength, CSA); non-climbing population; transfer inferential
& GRADE: moderate (non-climbing; isometric forearm applicability inferential). \\
