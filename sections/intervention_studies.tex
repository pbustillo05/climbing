...
\toprule
\textbf{Authors} & \textbf{Design / $n$} & \textbf{Protocol} & \textbf{Main finding} \\
\midrule
\endfirsthead
...
López-Rivera \& González-Badillo \cite{LRGB12} & 2-arm RCT; $n=16$ elite & 8~wk, 2$\times$/wk: max weight on 18~mm vs.\ minimal edge, same duration & Max-weight arm superior on strength; minimal-edge arm superior on endurance transfer \\[3pt]

Medernach et al.\ \cite{MKL15} & 2-arm RCT; $n=23$ adv.\ male & 4~wk, 3$\times$/wk fingerboard vs.\ bouldering only & FB: $+2.5$~kg grip, $+5$--7~s DH, $+26$~s IFH vs.\ control \\[3pt]

López-Rivera \& González-Badillo \cite{LRGB19} & 3-arm RCT; $n=26$ advanced & 8~wk, 2$\times$/wk: MaxHangs / IntHangs / Combo & Endurance: IntHangs $+45\%$ (ES~1.0), MaxHangs $+34\%$ (ES~0.6), Combo $+7\%$; MFS: MaxHangs $+28\%$ only \\[3pt]

Levernier \& Laffaye \cite{LL19} & Controlled; $n=23$ elite & 4~wk, 2$\times$/wk unilateral hangs (half-crimp $+$ slope) & $+8\%$ MFS; $\uparrow$RFD; control unchanged \\[3pt]

Mundry et al.\ \cite{MSA+21} & 3-arm RCT; $n=30$ int--adv & 8~wk, 2--3$\times$/wk: added weight on 20~mm / progressive edge reduction / control & Added weight $>$ control ($p=0.032$, ES~$0.36$); edge reduction $=$ control \\[3pt]

Devise et al.\ \cite{DO22} & 4-arm RCT; $n=54$ experienced & 4~wk, 2$\times$/wk: F100 / F80 / F60 / control; 12~mm instrumented hold & F100$+$F80: MFS $+12$--14\%; F80 stamina score: 29.5\%$\to$60.4\% (+31~pp); F60: 32.6\%$\to$53.5\% (+21~pp); F80 improves all three outcomes \\[3pt]

Hermans et al.\ \cite{HO22} & 2-arm RCT; $n=35$ int--adv & 10~wk, 2$\times$48~min/wk progressive repeaters (BM1000, 23~mm) vs.\ climbing only & Peak force $+17\%$, avg force $+18\%$ (within-group); RFD $+28\%$ within HBT group but between-group difference not significant ($p=0.056$); DH $+12\%$ within HBT \\[3pt]

Dindorf et al.\ \cite{DO24} & 2-arm RCT; $n=18$ completers (14M/4F) int--adv & 7~wk: NMES-assisted fingerboard 2$\times$/wk vs.\ conventional fingerboard 3$\times$/wk & NMES non-inferior to conventional training for finger flexor endurance despite ${\sim}33\%$ lower volume; no significant between-group difference \\[3pt]

Gilmore et al.\ \cite{GKAO24} & Retrospective cohort; $n=527$ Crimpd users & Abrahangs ($\geq3\times$/wk, ${\sim}40\%$ MFS) vs.\ Max Hangs ($\geq0.5\times$/wk, 85--95\%) vs.\ Both vs.\ Climbing Only & Abrahangs $+2.5\%$, Max Hangs $+3.2\%$, Both $+5.8\%$, Climbing Only $+0\%$ strength-to-weight; self-selected, self-reported loads \\[3pt]

Langer et al.\ \cite{LAS24} & RCT; $n=31$ female ($n=26$ completed) & 5~wk: off-wall (fingerboard, BM1000, progressive levels) vs.\ on-wall vs.\ control & Trends toward fingerboard superiority on strength (BF$_{10}=1.16$, anecdotal); on-wall preferred for enjoyment; no significant between-group strength differences \\[3pt]

Ruiz-López et al.\ \cite{RLPCBS25} & RCT; $n=11$ experienced (of 20 enrolled) & 4~wk: weighted dead-hang (HIMA, 85\%, 14~mm) vs.\ active-pull dynamometer (PIMA) & Both groups significant gains in peak force, RFD$_{200}$, and max hang time ($p \leq 0.05$); no between-group differences; PIMA reduced hand asymmetry \\[3pt]
...
\toprule
\textbf{Authors} & \textbf{Allocation conceal.} & \textbf{Participant blinding} & \textbf{Dropout / ITT} & \textbf{PEDro} & \textbf{Overall} \\
\midrule
\endfirsthead
...
López-Rivera \& González-Badillo \cite{LRGB12} & Unclear & Not feasible & Not reported & --- & High \\[2pt]
Medernach et al.\ \cite{MKL15} & Unclear & Not feasible & Minimal & 5 & High \\[2pt]
Levernier \& Laffaye \cite{LL19} & Unclear & Not feasible & Not reported & 5 & High \\[2pt]
López-Rivera \& González-Badillo \cite{LRGB19} & Unclear & Not feasible & 32\% (38→26) & --- & High \\[2pt]
Mundry et al.\ \cite{MSA+21} & Unclear & Not feasible & 10\% (30→27) & 6 & High \\[2pt]
Devise et al.\ \cite{DO22} & Unclear & Not feasible & Not reported & 7 & Moderate \\[2pt]
Hermans et al.\ \cite{HO22} & Unclear & Not feasible & Not reported & 6 & High \\[2pt]
Dindorf et al.\ \cite{DO24} & Unclear & Not feasible & 18/32 enrolled (44\%) & --- & High \\[2pt]
Langer et al.\ \cite{LAS24} & Stated & Not feasible & 16\% (31→26) & --- & Moderate \\[2pt]
Ruiz-López et al.\ \cite{RLPCBS25} & Unclear & Not feasible & 45\% (20→11) & --- & High \\[2pt]
Gilmore et al.\ \cite{GKAO24} & N/A (obs.) & N/A & Self-selected app users & --- & High (obs.) \\[2pt]
\end{longtable}
}% end footnotesize
